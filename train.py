...
def parse_args() -> argparse.Namespace:
    ...
    # Time-of-day filter + finetune support (exp/hour-window-finetune).
    # The filter is applied row-wise on the impression timestamp before
    # tensor conversion; supports cross-midnight intervals so the
    # EDA-confirmed test window (Sun 23:40:35 -> Mon 01:13:44, see
    # HANDOFF §1.6) can be matched exactly via spec "23:40-1:13".
    parser.add_argument('--hour_minute_filter', type=str, default='',
                        help='Row-wise time-of-day filter in "H:MM-H:MM" '
                             'format. Supports cross-midnight (start > end). '
                             'Example: "23:40-1:13" keeps rows whose '
                             '`timestamp % 86400` is in [85235, 86399] U '
                             '[0, 4424]. Empty = no filter.')
    parser.add_argument('--finetune_from', type=str, default='',
                        help='Path to a model.pt produced by a prior training '
                             'run. When set, load it into the model with '
                             'strict=True before training begins.')
    parser.add_argument('--save_every_epoch', action='store_true', default=False,
                        help='Persist a per-epoch ckpt under '
                             ...
                             'runs where val on a tiny filtered subset can '
                             'mis-rank epochs vs. true platform score.')
...
def _parse_hour_minute_filter(spec: str) -> Optional[Tuple[int, int]]:
    ...
    Both endpoints are inclusive seconds-of-day in [0, 86399]. The spec
    supports cross-midnight intervals: when start > end the filter is
    ``[start, 86399] U [0, end]`` (handled in dataset.PCVRParquetDataset).
    Returns ``None`` when ``spec`` is empty.
    """
    if not spec:
        return None
    if '-' not in spec:
    ...
    return (to_sec(a.strip()), to_sec(b.strip()))
...
def main() -> None:
    ...
    if time_filter is not None:
        logging.info(f"Time-of-day filter active: sod in "
                     f"[{time_filter[0]}, {time_filter[1]}] "
                     f"(spec={args.hour_minute_filter!r}, cross-midnight={'yes' if time_filter[0] > time_filter[1] else 'no'})")
    ...
    # Finetune: load a starting checkpoint with strict shape matching.
    # Must happen after .to(device) so loaded tensors land on the same
    # device. Strict=True ensures any architectural drift between the
    # source ckpt and this run fails loudly rather than silently
