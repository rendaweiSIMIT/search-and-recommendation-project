...
def parse_args() -> argparse.Namespace:
    ...
    # InterFormer-style Cross Arch: seq -> non-seq feedback. Adds a
    # CrossArchSummarizer that pools each domain's evolved seq tokens via
    # (CLS-attention + recent-K mean), projects through a gated MLP, and
    # adds the result as a residual to the pooled output before the
    # classifier. Captures the missing s2n direction in the baseline
    # architecture (Meta CIKM 2025 paper).
    parser.add_argument('--use_cross_arch', action='store_true', default=False,
                        help='Enable InterFormer-style seq -> NS Cross Arch '
                             'feedback head (CLS-attn + recent-K mean -> '
                             'residual to pooled output)')
    parser.add_argument('--no_cross_arch', dest='use_cross_arch',
                        action='store_false',
                        help='Disable Cross Arch feedback')
    parser.add_argument('--cross_arch_recent_k', type=int, default=8,
                        help='Number of most-recent tokens averaged in the '
                             'Cross Arch recent-K pooling (default 8)')
...
def main() -> None:
    ...
    model_args = {
        ...
        "use_cross_arch": args.use_cross_arch,
        "cross_arch_recent_k": args.cross_arch_recent_k,
    }
