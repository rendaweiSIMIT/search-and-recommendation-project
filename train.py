...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--paired_pool_fids', type=str,
                        ...
                        help='Comma-separated user_int / user_dense fids whose '
                             ...
                             'branch: 89-91 only (62-66 stay on baseline mean '
                             'pool because their raw-counter dense values span '
                             'up to ~1e9; signed_log1p compression still '
                             'leaves a [0, log(1e9)~21] range, over which '
                             'softmax degenerates to argmax/hard-attention and '
                             'discards most categories. Empirically the 5-fid '
                             'exp/paired-pool-62-66 branch scored BELOW '
                             'baseline, while 89-91 -- already-normalized '
                             'scores in [-0.92, 0.92] -- produce a healthy '
                             'soft-attention spread). Pass an empty string '
                             'to disable all paired pooling.')
