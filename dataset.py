...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        Args:
            ...
            time_of_day_filter: ``(start_sec, end_sec)`` inclusive seconds-of-day
                window applied row-wise on the ``timestamp`` column (using
                ``ts % 86400``). Rows outside the window are dropped before
                tensor conversion. ``None`` (default) keeps every row.
        """
        super().__init__()
